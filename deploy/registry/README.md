...
确认成功会在再次复核路径和 ACL 后原子替换状态文件，删除配对码和两个独立 raw secret 字段；WSS 设备 secret 与 disclosure bridge secret 分别封装进 `dsh1`、`dshb1` token。导出的六个变量可供只启用鉴权、在线状态与重连的 `harness-registry-connection.example.patch.yml` 使用，也可供 `harness-production-publication.example.patch.yml` 调用内建 HTTPS bridge 完成权限、容量和数据密钥提供。Registry PostgreSQL 层已经装配正文投影 provider，当前完整模板也已接入 Web 侧 WSS Session 导入与刷新。自动问题消费者尚未接入，因此该模板不要求模型 provider、model 或模型凭据，也不会执行远程工具。两个文件都含长期敏感凭据，必须只允许 Harness 服务账号读取，不能上传、发送或提交到 Git；需要重做时请先在 Registry 撤销旧设备，再由运维人员明确移走旧文件。

升级前遗留的 V1 待确认状态仍可完成 `confirm`，V1 已确认状态仍可 `export-env`；两者都只保留原有 `dsh1`，导出严格五项变量，仅支持 connection-only WSS。工具不会把旧 secret 复用为 `dshb1`，也不会把 V1 状态静默升级为可发布状态。要启用 HTTPS disclosure publication，必须撤销旧设备并重新绑定。`check-production-environment.mjs harness`／`all` 对应公网 publication，固定要求新六项设备配置，不要求尚未接入的自动问题消费者所需模型变量。它还要求 `DSH_REGISTRY_DISCLOSURE_BRIDGE_URL` 精确指向同一 `REGISTRY_DOMAIN` 的 `/a2a/v1/disclosure-publication` HTTPS 地址，不接受显式端口、凭据、查询或片段。
...
目标源码和已构建 web-app／session-controller 还会用各自的 app-boot 与 schema 分别解析实际 connection-only overlay 和 publication overlay，并执行 Web 跨字段组合校验。最后已构建 CLI 会为两层 overlay 分别创建临时工作目录与临时 `DSH_HOME` 执行 `--dump-config`：前者必须只合成生产连接，后者必须同时合成生产连接、披露发布、确定会话导入与刷新，且不得混入尚未接入的自动问题消费者、测试或 loopback provider；完成后删除临时目录，不写目标 Harness 的真实 profile。
