...
test('Windows Harness runtime preparer declares the static release and ACL gates', () => {
  ...
  assert.match(preparer, /未由输入 tarball 声明的 DeepSeek 包/u)
  assert.match(preparer, /受保护输入声明的外部 DeepSeek 依赖必须固定精确版本/u)
  assert.match(preparer, /registryDeepseekDependencies/u)
  assert.match(preparer, /Production 运行包必须提供受保护的 ReleaseSigningPrivateKey/u)
  assert.match(preparer, /RSA-PSS-SHA256/u)
  assert.match(preparer, /runtime-signature\.json/u)
  assert.match(preparer, /\$stagingName = "\.dshrt-\$\(\[Guid\]::NewGuid\(\)\.ToString\('N'\)\)"/u)
  ...
  assert.match(launcher, /Production 启动必须提供包外受保护的 TrustedReleasePublicKey/u)
  assert.match(launcher, /function Assert-ReleaseSignature/u)
  assert.match(launcher, /RSA-PSS-SHA256/u)
  assert.match(launcher, /runtime-signature\.json/u)
