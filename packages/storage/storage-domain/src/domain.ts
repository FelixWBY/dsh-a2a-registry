...
import type { KvRecordWrite, KvUnit } from '@deepseek-ai/dsh-storage'
...
/** A record replacement whose table selects its key and value types. */
export type DomainRecordWrite<S extends DomainSpec> = {
...
export interface Domain<S extends DomainSpec> {
  ...
  /**
   * Atomically upsert a batch at one write-chain slot. Present only when
   * the backend provides atomic batches. All new values become readable
   * before notifications emit in input order. Rejections publish no memory
   * or events, but storage may have committed; reopen to determine its state.
   * Duplicate table/key pairs reject before I/O. Empty batches emit nothing
   * and perform no I/O; admission after close still rejects.
   * @param writes - Descriptors captured before queueing; values must remain immutable.
   * @returns Resolution after durability, memory publication and notifications.
   */
  putMany?(writes: readonly DomainRecordWrite<S>[]): Promise<void>
...
export class DomainImpl {
  /** Domain name from the spec. */
  readonly name: string
  /** Atomic record batches, present only when the opened unit provides them. */
  declare readonly putMany?: (writes: readonly KvRecordWrite[]) => Promise<void>
  ...
  constructor(
    ...
    if (unit.putRecords !== undefined) {
      const putRecords = unit.putRecords.bind(unit)
      this.putMany = (writes) => {
        const batch = writes.map(({ table, key, value }) => ({ table, key, value }))
        return this.enqueue(async () => {
          if (batch.length === 0) return
          const installs = batch.map(write => ({ write, table: this.requireTable(write.table) }))
          await putRecords(batch)
          for (const { write, table } of installs) table.install(write.key, write.value)
          for (const { write } of installs) {
            this.emitChanged({ domain: this.name, table: write.table, key: write.key, operation: 'put', value: write.value })
          }
        })
      }
    }
  ...
  table(name: string): KvTable<string, unknown> {
    return this.requireTable(name)
  }

  private requireTable(name: string): KvTableImpl<string, unknown> {
  ...
  private emitChanged(change: DomainChanged): void {
    try {
      ...
      try {
        ...
        // A synchronous log exporter failure cannot reject an already-committed write.
      }
    }
  }
...
class KvTableImpl<K extends string, V> implements KvTable<K, V> {
  ...
  /** Install an acknowledged batch value before any batch change notification. */
  install(key: K, value: V): void {
