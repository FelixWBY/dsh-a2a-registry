...
class SingleJsonUnit implements KvUnit {
  ...
  async putRecords(records: readonly KvRecordWrite[]): Promise<void> {
    ...
    for (const { table, key, value } of batch) {
      ...
      seen.add(identity)
      let target = touched.get(table)
      if (target === undefined) {
        target = new Map(this.records(table))
        tables.set(table, target)
        touched.set(table, target)
      }
  ...
  private publish(state: UnitState = this.state): Promise<void> {
    const write = writeAtomic(this.path, serialize(this.descriptor.name, state))
