...
/** One immutable record replacement within an atomic unit batch. */
export interface KvRecordWrite {
  /** Declared table receiving the record. */
  readonly table: string
  /** Opaque record key, subject to the unit layout's key rules. */
  readonly key: string
  /** JSON-serializable value; callers must not mutate it while the unit retains it. */
...
export interface KvUnit {
  ...
  /**
   * Atomically upsert every record in this unit, or none. Only capable
   * backends expose this method; there is no sequential-write fallback.
   * A rejection can follow a committed write: close and reopen before
   * deciding the durable result. Duplicate table/key pairs reject before
   * any write; an empty batch writes nothing, but still rejects after close.
   * @param records - Captured record descriptors; values follow the immutable-value contract.
   * @returns Resolution after the complete batch is durable.
   */
