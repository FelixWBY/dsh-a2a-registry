 ...
 * `prefers-color-scheme`, and publishes immutable snapshots. The optional
 * document presenter applies snapshots for the composition's root owner. The Host
 * settings scope, when selected, loads and stores the preference in the user-settings
 * document; session mode retains it only in memory. The plugin also registers the Appearance preference row into the
...
export interface ThemeSnapshot {
  /** Current preference (may be `system`); persisted only with a Host settings scope. */
...
export class ThemeRuntime {
   ...
   * @param host - durable preference scope owned by the same plugin; absent means process-local defaults and writes.
   */
  constructor(ctx: ClientContext, host?: SettingsScope<ThemeSettings>) {
    this.ctx = ctx
    this.host = host
    this.preference = DEFAULT_PREFERENCE
    this.fontSize = host === undefined ? DEFAULT_FONT_SIZE : bootstrapFontSize()
  ...
  /**
   * Apply the live theme to the document for the composition's sole root presenter.
   * The caller owns the returned disposer through ctx.effect; theme-owner disposal also releases it.
   * Cleanup removes this presentation's palette, font size, token overrides and metadata, not unrelated DOM values.
   * @returns Idempotent disposer that stops synchronization and retracts the presentation.
   * @throws When this runtime already has a document presenter.
   */
  present(): () => Promise<void> {
    if (this.presenting) throw new Error('theme document presentation already has an owner')
    return this.ctx.effect(() => {
      const presenter = new ThemePresenter()
      presenter.apply(this.getTheme())
      this.presenting = true
      const off = this.ctx.on('theme/change', (snapshot) => { presenter.apply(snapshot) })
      return () => {
        off()
        this.presenting = false
        presenter.dispose()
      }
    }, 'ui-theme: document presentation')
  }
   ...
   * Built-in preferences are written through the settings scope when present and every
   ...
   * entry. Accepted values are written through the settings scope when present and emit
   * `theme/change`.
 ...
 * ran, so the initial snapshot matches first paint and the root's presenter
...
/** Browser preference ownership; session preferences exist only in this mounted runtime. */
export interface Config {
  /** Host-backed settings, or browser-process-only selection without settings transport. */
  preferenceSource: 'host' | 'session'
}

/** Validated Client preference ownership; existing Web compositions use Host settings. */
export const Config: z<Partial<Config>, Config> = z.object({
  preferenceSource: z.union(['host', 'session']).default('host'),
})

/** Slots and locale are required in either preference mode. */
 ...
 * @param config - validated preference owner; Host mode waits for the real transport and settings scope.
 */
export function apply(ctx: ClientContext, config: Config = Config({})): void {
  if (config.preferenceSource === 'host') {
    ctx.inject(['remote', 'settingsScope'], (hostCtx) => {
      mount(hostCtx, hostCtx.settingsScope.bind<ThemeSettings>({ namespace: THEME_SETTINGS_NAMESPACE }))
    })
  } else {
    mount(ctx)
  }
...
/** Provide all theme contributions for the selected preference owner's lifetime. */
function mount(ctx: ClientContext, host?: SettingsScope<ThemeSettings>): void {
