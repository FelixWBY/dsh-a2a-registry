...
import { parseClientPublicConfig } from './client/config.ts'
import type { ClientPublicConfig } from './client/config.ts'
...
export type {
  BootManifest, BootModuleRow, BootPluginRow, ClientPublicConfig, WebBootBatch, WebBootBatchPhase, WebBootEntry, WebBootGraph,
...
interface WebPluginRecord {
  entry: WebBootEntry
  /** Sole explicit public configuration contribution, valid only for this exact Loader source record. */
  clientConfig?: { value: ClientPublicConfig }
...
export class ClientModuleRegistry extends Service {
  ...
  /**
   * Publish explicit public configuration for one currently composed Client package.
   * The caller owns the disposer through ctx.effect. Duplicate owners and unknown targets throw.
   * Artifact rebuilds retain this contribution; target removal or Loader source replacement discards it.
   * Changes affect subsequent index responses, not already-open pages' initial Loader configuration.
   * Failed graph recomposition leaves contribution ownership and the published graph unchanged, including on disposal.
   * @param id - Exact package id in the current graph, not a Host Loader alias.
   * @param config - Public JSON fields only; Host entry configuration is never forwarded implicitly.
   * @returns Idempotent disposer removing only this contribution and synchronously notifying graph listeners.
   */
  registerClientConfig(id: string, config: ClientPublicConfig): () => void {
    const record = this.table.get(id)
    if (record === undefined || !this.composed.entries.some(row => row.id === id && row.rev === record.entry.rev)) {
      throw new Error(`client-modules: public config target "${id}" is not in the current graph`)
    }
    if (record.clientConfig !== undefined) throw new Error(`client-modules: public config target "${id}" already has an owner`)
    const contribution = { value: parseClientPublicConfig(id, config) }
    record.clientConfig = contribution
    try {
      this.composed = this.compose()
    } catch (error) {
      delete record.clientConfig
      throw error
    }
    this.notifyGraphChanged()
    return () => {
      if (this.table.get(id) !== record || record.clientConfig !== contribution) return
      delete record.clientConfig
      try {
        this.composed = this.compose()
      } catch (error) {
  ...
  private compose(): WebBootGraph {
    const entries = orderByModuleGraph([...this.table.values()].map(record => record.clientConfig === undefined
      ? record.entry : { ...record.entry, config: record.clientConfig.value }))
