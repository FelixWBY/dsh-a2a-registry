...
import { parseClientPublicConfig } from './config.ts'
import type { ClientPublicConfig } from './config.ts'

export type { ClientPublicConfig } from './config.ts'
...
export interface WebBootEntry {
  ...
  external?: string[]
  /** Explicitly registered public Client configuration; absent leaves plugin defaults unchanged. */
  config?: ClientPublicConfig
}
...
export interface BootPluginRow {
  ...
  immediately: boolean
  /** Validated public configuration passed to the Client Loader, never copied from Host entry options. */
  config?: ClientPublicConfig
}
...
export function parseBootManifest(wire: unknown): BootManifest {
  ...
  for (const value of graph.entries as unknown[]) {
    ...
    const config = row.config === undefined ? undefined : parseClientPublicConfig(subject, row.config)
