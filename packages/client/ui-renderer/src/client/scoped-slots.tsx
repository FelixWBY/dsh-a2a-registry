...
import {
  ...
  useScopeBinding, useOptionalScopeBinding,
} from './bindings.tsx'
...
function SlotOutlet({ slotKey, ownerProps, opts }: {
  ...
  const scopeBinding = useOptionalScopeBinding()
...
function renderOutletContent(
  ...
  ownerProps: object,
  opts: (RenderOpts & ChainRenderOpts) | undefined,
  scopeBinding: StandardSourceBinding | null,
): ReactNode {
  ...
  if (!spec) return null
  if (spec.scope !== 'root' && scopeBinding === null) {
...
/** Bind an installed Session adapter without requiring one for a root-only application. */
function RootScopeProvider({ children }: { children: ReactNode }) {
  const host = useHost()
  observableHook(host.scopeRevision)(value => value)
  if (host.scope('session') === undefined) return children
  return <ScopeProvider scope="session-maybe">{children}</ScopeProvider>
}
