...
export class LocaleRuntime {
   ...
   *
   * With a Host scope, the durable write happens even when the id already matches the active
...
/** Slot registration is required in either preference mode. */
...
export function apply(ctx: ClientContext, config: Config = Config({})): void {
  if (config.preferenceSource === 'host') {
    ctx.inject(['remote', 'settingsScope'], (hostCtx) => {
      mount(hostCtx, hostCtx.settingsScope.bind<LocaleSettings>({ namespace: LOCALE_SETTINGS_NAMESPACE }))
    })
  } else {
    mount(ctx)
  }
...
/** Provide all locale contributions for the selected preference owner's lifetime. */
function mount(ctx: ClientContext, host?: SettingsScope<LocaleSettings>): void {
